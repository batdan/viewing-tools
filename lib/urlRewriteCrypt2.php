 ...
 * code alpha juste après le nom de domaine
 *
 * Méthode avec cryptage alpha
 *
 * @author Daniel Gomes
 */
class urlRewriteCrypt2
{
	/**
	 * Attributs
	 */
	 ...
	 */
	public function __construct($http_host='', $request_uri='', $useSubDomain=true)
	{
		$this->_http_host = empty($http_host) ? $_SERVER['HTTP_HOST'] : $http_host;
		$this->_request_uri = empty($request_uri) ? $_SERVER['REQUEST_URI'] : $request_uri;
		...
		if ($useSubDomain && $this->_subDomain == '') {
			$this->redirHTTP(301);
		}
